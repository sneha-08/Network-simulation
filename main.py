...
def c_func_def(Net, nodes, multiplayer=packet_size*15):
    c_func = Net.copy()
    weights = []
    ...
    for i, k in Net.edges:
        if c_func[i][k]['weight'] == 0:
            ...
            c_func[i][k]['weight'] = max(c_func[i][k]['weight']*multiplayer, int(avg_weights)*multiplayer)
    return c_func
    ...
    for i in range(iterations):
        ...
        for i, k in temp_net.edges:
            if p < randint(1, 100) <= 100:
                ...
                if not nx.is_connected(temp_net):
...
def delta_topology(Net, matrix_flow, c_func, iterations=10):
    ...
    for i in range(iterations):
        ...
        while e1 == e2 or temp_net.has_edge(e1, e2):
            e1 = randint(0, len(temp_net)-1)
            e2 = randint(0, len(temp_net)-1)
...
def net(edges=29, nodes=20):
    matrix_flow = [[randint(0, 10) if i != k else 0 for k in range(nodes)] for i in range(nodes)]
    print("Starting matrix_flow")
    print(np.matrix(matrix_flow))
    Net = create_network_graph(edges, nodes)
    a_func_def(Net, matrix_flow, nodes)
    c_func = c_func_def(Net, nodes)
    show_graph(Net)
    show_graph(c_func)
